  ...
  def __init__(self, inner_optimizer, dynamic=True, initial_scale=None,
    ...
    if not isinstance(inner_optimizer, optimizer_v2.OptimizerV2):
      if isinstance(inner_optimizer, optimizer_experimental.Optimizer):
        ...
        raise TypeError(
            'You passed an instance of the new experimental optimizer, '
            '`optimizer_experimental.Optimizer`, to LossScaleOptimizer, but '
            'only the classic optimizers subclassing from '
            '`tf.keras.optimizers.Optimizer` can be passed. Please use '
            '`loss_scale_optimizer.LossScaleOptimizerV3` instead of '
            '`tf.keras.mixed_precision.LossScaleOptimizer`, as the former '
            'supports wrapping instances of the new experimental optimizer. '
            'Got optimizer: %s' % inner_optimizer)
    ...
    if not isinstance(inner_optimizer, optimizer_experimental.Optimizer):
      if isinstance(inner_optimizer, optimizer_v2.OptimizerV2):
        ...
        raise TypeError(
            ...
            'LossScaleOptimizerV3, but only the new experimental optimizer '
            'defined in keras/optimizer_expeirmental/optimizer.py can be '
            'passed. Please use `tf.keras.mixed_precision.LossScaleOptimizer` '
            'instead of LossScaleOptimizerV3, as the former supports '
            '`tf.keras.optimizer.Optimizer`s. Got optimizer: %s'
            % inner_optimizer)
      raise TypeError('"inner_optimizer" must be an instance of '
                      'Optimizer, but got: %s.' % inner_optimizer)
    if not isinstance(dynamic, bool):
      # Catch errors if a user incorrectly passes a string or float to the
      # second argument argument, as this is commonly done for
      # LossScaleOptimizerV1.
