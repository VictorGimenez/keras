...
class GetOrCreateLayerTest(tf.test.TestCase):
  ...
  def test_get_or_create_layer_eager(self):
    ...
    self.evaluate(tf.compat.v1.global_variables_initializer())
    out1 = self.evaluate(out1)

    model1 = layer.submodules[0]._layers["nested_layer/dense_model"]

    out2 = layer(tf.expand_dims(x, 0))
    out2 = self.evaluate(out2)
    ...
    model2 = layer.submodules[0]._layers["nested_layer/dense_model"]
