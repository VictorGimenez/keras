...
def track_tf1_style_variables(method):
  ...
  Caveats:
    ...
    * This decorator only adds implicit tracking for legacy tf1-style
      ...
      inside the decorated method, see `keras.utils.get_or_create_module`.
...
def get_or_create_layer(name, create_layer_method):
  """Use this method to track nested keras models in a decorated method.
  ...
  class WrappedDenseLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
      ...
      return model(inputs)
  ```
  The inner model creation should be confined to its own function, which should
